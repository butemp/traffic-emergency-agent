...
审核标准：
0. 完整性优先，候选方案篇幅长不是问题；除非明显重复或无关，不要因为内容复杂而扣分
1. 是否满足标准化应急指挥方案格式，尤其是固定章节和顺序是否完整
2. 是否使用建议性表述，不能谎称已经通知队伍、下达指令、启动真实行动
3. 是否覆盖核心内容：事件概述、响应定级、指挥架构、预警发布、处置行动、资源调度、风险提示、依据引用
4. 指挥架构是否覆盖应急管理部门、公安交管部门、消防救援部门、医疗救援部门、高速/道路运营管理单位和专家技术支持组
5. 资源调度是否说明资源来源单位或出发地、调度路径、预计到达时间、联系人和电话；如果无法规划路径，是否明确说明原因
6. 处置行动是否包含涉险人员二次排查、事故现场其他伤员排查、检伤分类和转运、家属联络安抚、现场警戒和二次事故防范
7. 资源类别是否使用中文名称，不能直接输出 WARNING、PPE、SIGN、VEHICLE、RESCUE、COMMS 等内部编码
8. 如上下文中已有专家检索结果，指挥架构或专家技术支持部分是否写出专家姓名、单位、专业方向和建议支持方式
9. 如果某些信息确实缺失，是否明确写了"暂未获取"或"待现场确认"，而不是直接漏掉
10. 是否存在明显空洞、缺少关键依据、内容前后矛盾或过度简略的问题
11. Markdown 格式是否规范：所有表格是否有表头行和分隔行（|---|---|），每行列数是否与表头一致，表格前后是否留空行，是否存在格式断裂或未闭合的表格
...
14. 资源调度方案是否说明了物资用途：是否包含“关键物资用途说明”，写清物资名称、来源资源、用途、使用位置、调度理由和注意事项；是否包含“资源覆盖与缺口分析”，写清已覆盖、不足、缺口和补充建议
15. 风险提示与注意事项是否足够详细：是否分为安全风险、处置风险、衍生风险三类；是否至少 9 条，建议 10-12 条；每条风险是否写清风险描述、触发条件、影响后果、应对措施、责任单位、监测指标、升级条件；内容是否结合本次事故的实际情况而非泛泛而谈
16. 必须逐章审核 9 个章节的详细度。尤其是“指挥架构”“处置行动方案”“资源调度方案”“风险提示与注意事项”，不能只给总评。每章都要判断是否内容充实、是否能直接支撑指挥员决策。
17. 资源调度中的“第一梯队/第二梯队/外部资源补充/专家技术支持/关键物资用途说明/资源覆盖与缺口分析”是否使用 #### 小标题，而不是作为普通段落夹在两个表格之间。
