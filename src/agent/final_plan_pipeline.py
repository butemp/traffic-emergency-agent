...
SECTION_SPECS: tuple[SectionSpec, ...] = (
    ...
    SectionSpec(
        ...
        instructions=(
            ...
            "字段名称不能改写。"
            "如调用过 search_experts 且有返回，可在表格后追加一段 **专家库支持**（依据 search_experts 候选名单），逐人列出姓名、单位、专业方向和建议支持方式。"
        ...
        example=(
            ...
            "\n"
            "**专家库支持**（依据 search_experts 候选名单）：\n"
            "\n"
            "- 廖俊锋（广西交通安全研究中心）：应急管理、安全管理 — 建议由指挥部办公室或值班人员人工联系参与远程会商或现场技术支持\n"
            "- 郑屈（广西交通安全研究中心）：安全应急管理 — 建议人工联系参与技术支持\n"
            "- 林静（南宁轨道交通集团有限责任公司）：轨道交通安全质量监管 — 建议人工联系参与技术支持\n"
        ),
    ),
    SectionSpec(
    ...
    SectionSpec(
        ...
        instructions=(
            ...
            "这 5 个字段会被 API 结构化提取，列名不能改写。"
            "引用章节/模块列必须填 get_emergency_plan 返回的 hit_path（中文章节路径，如 "
            "'应急响应.处置措施.Ⅱ级应急响应处置措施' 或 '附件.附件2 公路水运工程生产安全事故响应分级和启动条件'），"
            "精确到原文档章节路径，不要写成笼统的'第X节'或'附件X'。"
        ...
        example=(
            ...
            "| 工具结果 | evaluate_incident_severity | 事件定级 | 判定为重大级（Ⅱ级），置信度0.85 | 事件等级判定 |\n"
            "| 工具结果 | search_emergency_resources | 资源搜索 | 搜索到5个仓库/队伍，覆盖警示防护、清障救援、夜间作业等能力 | 物资装备与调度填充 |\n"
            "| 工具结果 | search_experts | 专家检索 | 匹配5位专家，涵盖应急管理、安全生产、轨道交通等方向 | 应急组织机构-专家库支持 |\n"
            "| 工具结果 | gaode 地理编码/天气 | 地理与气象 | 经纬度108.32, 22.84；中雨22℃能见度500米 | 事件现场基本情况字段 |\n"
        ),
    ),
)
