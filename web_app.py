...
SECTION_MIN_LENGTHS = {
    ...
    "三、应急组织机构": 350,
    "四、物资装备与调度": 250,
    "五、处置流程建议（包括后期处置、新闻发布）": 350,
    "六、次生风险": 250,
    "七、引用依据": 200,
}

SECTION_DETAIL_REQUIREMENTS = {
    "一、事件现场基本情况": ("事件地点", "天气情况", "事件简述", "周边环境", "主要影响"),
    "二、预案匹配与组织预警和响应": ("匹配预案", "事件等级", "预警发布", "启动响应", "判断依据"),
    "三、应急组织机构": ("工作组", "牵头单位", "主要职责", "现场指挥组", "综合协调组", "抢险处置组", "医疗救护组", "后勤保障组", "信息发布组", "专家组"),
    "四、物资装备与调度": ("所需物资", "推荐调度来源", "距离", "预计到达时间", "联系人信息", "资源缺口"),
    "五、处置流程建议（包括后期处置、新闻发布）": ("序号", "行动", "责任单位", "协同单位", "引用依据", "现场警戒", "二次排查", "新闻发布"),
    "六、次生风险": ("触发条件", "风险描述", "影响后果", "应对措施", "责任单位"),
    "七、引用依据": ("依据类型", "依据名称", "引用章节", "引用内容", "支撑决策"),
}
...
def build_output_format_retry_prompt() -> str:
    ...
    return (
        ...
        "0. 结构必须严格按【应急处置总览】+【应急处置详情】（7 个固定章节）输出，不能增删、不能换序；\n"
        "1. 必须按以下 7 个固定章节、固定顺序输出：\n"
        f"{section_text}\n"
        "2. 除第三节固定输出工作组表 + 专家库支持表两张独立表格外，每个章节只输出规范要求的那张固定字段表，不要添加额外子表（'事件基础信息'/'补充说明'/'第一梯队/第二梯队'/'关键物资用途说明'/'资源覆盖与缺口分析'/'三阶段子表'/'安全/处置/衍生风险三类'等都已经被砍掉）；\n"
        "3. 表头列名严格不能改写，API 会按列名结构化提取（错一个字段就解析不出来）：\n"
        "   - 一：字段、内容（5 行：事件地点/天气情况/事件简述/周边环境/主要影响）\n"
        "   - 二：字段、内容（5 行：匹配预案/事件等级/预警发布/启动响应/判断依据）\n"
        "   - 三：固定两张独立 Markdown 表格。第一张：工作组、牵头单位、主要职责（至少 7 个工作组：现场指挥组/综合协调组/抢险处置组/医疗救护组/后勤保障组/信息发布组/专家组）；第二张在'专家库支持'标题后输出：专家姓名、所在单位、专业方向、联系电话、建议支持方式\n"
        ...
        "4. 五、处置流程建议 必须包含涉险人员二次排查、其他伤员排查、家属联络安抚、二次事故防范、清障与道路恢复、信息报送（初报/续报/终报）、新闻发布、舆情监测、总结评估；\n"
        "5. 四、物资装备与调度 必须基于 search_emergency_resources 实际返回数据编写，不能编造仓库名/电话/物资；\n"
        "6. 七、引用依据 的'引用章节/模块'列必须填 get_emergency_plan 返回的 hit_path（如 '应急响应.处置措施.Ⅱ级应急响应处置措施'），精确到原文档章节路径，不要写笼统的'第X节'；\n"
        "7. 全文只能写建议性表述，不能写成'已通知/已派遣/已下达指令/已启动应急响应'；\n"
        "8. 资源类别只能用中文名称，不能直接输出 WARNING、PPE、SIGN、VEHICLE 等内部编码；\n"
        "9. 如已有专家检索结果，必须在'三、应急组织机构'工作组表后追加'专家库支持'独立 Markdown 表格，列名固定为专家姓名、所在单位、专业方向、联系电话、建议支持方式；严禁写成项目符号列表或普通段落；\n"
        "10. 缺失信息请明确写'暂未获取'或'待现场确认'，不要省略章节。\n"
        "\n"
        "【Markdown 格式硬性要求】\n"
        "- 所有表格必须有表头行和分隔行（即 |---|---|--- 格式），且列数一致；\n"
        "- 表格前后必须留空行；\n"
        "- 章节标题统一使用 ### 级别（如 ### 一、事件现场基本情况）；\n"
        "- 总览块用 ### 【应急处置总览】，详情块用 ### 【应急处置详情】；\n"
        "- 列表项统一使用 - 开头；\n"
        "- 不要出现未闭合的表格行或格式断裂。\n"
        "\n"
        "【次生风险章节硬性要求】\n"
        "- 整章只用一张 5 列表（触发条件、风险描述、影响后果、应对措施、责任单位），不要分三类子表；\n"
        "- 至少 5 条，建议 6-8 条，覆盖：二次事故、现场作业安全、伤员漏查、拥堵外溢、家属安抚、舆情扩散、资源不足等；\n"
        "- 应对措施要具体到动作（如'上游设置渐变式警戒区+锥桶渠化+爆闪灯'），不要写'加强管理'这种空话。\n"
        "\n"
...
def collect_final_plan_guardrail_issues(text: str, agent: Optional[Agent] = None) -> list[str]:
    ...
    if any(marker in text for marker in process_markers):
        issues.append("最终方案中混入了模型推理过程说明，应删除“第一步/第二步”等过程性文字，只保留标准 7 章节方案。")

    if not has_standard_plan_structure(text):
        issues.append("方案未满足固定 7 章节结构或章节顺序不正确。")
...
def collect_section_detail_issues(text: str) -> list[str]:
    ...
    issues.extend(_check_command_structure_detail(_extract_section(text, "三、应急组织机构")))
    issues.extend(_check_action_plan_detail(_extract_section(text, "五、处置流程建议（包括后期处置、新闻发布）")))
    issues.extend(_check_reporting_section_detail(_extract_section(text, "五、处置流程建议（包括后期处置、新闻发布）")))
    issues.extend(_check_reference_section_detail(_extract_section(text, "七、引用依据")))
    return issues
...
def _check_command_structure_detail(section: str) -> list[str]:
    ...
    if missing:
        issues.append("三、应急组织机构不够完整，缺少：" + "、".join(missing))

    row_count = _count_table_data_rows(section)
    if row_count < 7:
        issues.append("三、应急组织机构工作组数量不足，建议至少列出 7 个工作组并写清牵头单位、参与单位、职责。")

    if "专家库支持" in section:
        expert_part = section.split("专家库支持", 1)[1]
        if "| 专家姓名 | 所在单位 | 专业方向 | 联系电话 | 建议支持方式 |" not in expert_part:
            issues.append("三、应急组织机构的专家库支持必须单独使用 Markdown 表格，列名为：专家姓名、所在单位、专业方向、联系电话、建议支持方式。")
        if re.search(r"(?m)^\s*[-•]\s+", expert_part):
            issues.append("三、应急组织机构的专家库支持不能使用项目符号列表，必须改为独立 Markdown 表格。")
...
def _check_action_plan_detail(section: str) -> list[str]:
    ...
    if action_count < 10:
        issues.append(f"五、处置流程建议行动项不足，当前约 {action_count} 条，建议至少 10 条覆盖警戒/救治/二次排查/家属/清障/报送/新闻发布/总结评估等动作。")

    required_actions = ("现场警戒", "二次排查", "家属", "清障", "新闻发布", "舆情")
    missing_actions = [item for item in required_actions if item not in section]
    if missing_actions:
        issues.append("五、处置流程建议缺少关键动作：" + "、".join(missing_actions))
...
def _check_reporting_section_detail(section: str) -> list[str]:
    ...
    if len(missing) > 2:
        issues.append("五、处置流程建议（信息报送/新闻发布部分）缺少关键内容：" + "、".join(missing))

    if _count_table_data_rows(section) < 5:
        issues.append("五、处置流程建议内容偏少，建议至少列出初报、续报、终报、新闻发布、舆情回应等事项。")
...
def _check_reference_section_detail(section: str) -> list[str]:
    ...
    if "《" not in section or "》" not in section:
        issues.append("七、引用依据未列出明确的预案或法规名称。")
    if "章节" not in section and "第" not in section and "." not in section:
        issues.append("七、引用依据未写清引用章节或条款位置（应填 get_emergency_plan 返回的 hit_path）。")
    if _count_table_data_rows(section) < 2:
        issues.append("七、引用依据条目过少，建议用表格列出预案、工具结果、案例或法规依据及其支撑的决策。")
...
def build_pre_output_tool_prompt(agent: Agent, issues: list[str]) -> str:
    ...
    lines.extend([
        "",
        "重要提醒：最终方案的'四、物资装备与调度'必须基于以上工具的实际返回数据来编写，"
        "包括每个资源/能力的所需物资、推荐调度来源、距离、预计到达时间、地点+联系人信息、资源缺口等，不能凭空编造。",
    ])
...
def build_final_review_retry_prompt(
    ...
    return (
        ...
        "2. 必须保持【应急处置总览】+【应急处置详情】7 个固定章节和顺序；\n"
        "3. 除三、应急组织机构固定输出工作组表 + 专家库支持表两张独立表格外，每章只输出规范要求的那张固定字段表，不要添加额外子表；\n"
        "4. 只能使用建议性表述，不能写成已经通知、已经下达、已经派遣；\n"
        "5. 三、应急组织机构 必须覆盖应急管理、消防救援、公安交管、医疗救援和专家技术支持等 7 个工作组；\n"
        "6. 四、物资装备与调度 必须基于 search_emergency_resources 实际数据；\n"
        "7. 五、处置流程建议 必须包含涉险人员二次排查、其他伤员排查、家属联络安抚、新闻发布、舆情监测、总结评估；\n"
        "8. 资源类别必须用中文名称，不能直接输出 WARNING、PPE、SIGN、VEHICLE 等内部编码；\n"
        "9. 如已有专家检索结果，必须在三、应急组织机构工作组表后追加'专家库支持'独立 Markdown 表格，列名固定为专家姓名、所在单位、专业方向、联系电话、建议支持方式；严禁写成项目符号列表或普通段落；\n"
        "10. 对暂时缺失的信息要明确写'暂未获取'或'待现场确认'；\n"
        "11. 回复末尾必须附上 agent_control，并设置 final_output=true；\n"
        "12. 不要输出'第一步：分析工具结果''第二步：处置方案生成'等过程说明，直接给标准 7 章节方案。\n\n"
        "Markdown 格式要求：\n"
        "- 所有表格必须有表头行和分隔行（|---|---|），且每行列数与表头一致；\n"
        "- 表格前后必须留空行；\n"
        "- 章节标题用 ### 级别；总览块用 ### 【应急处置总览】，详情块用 ### 【应急处置详情】；\n"
        "- 不要出现未闭合的表格行或格式断裂。\n\n"
        "各章节固定表头（API 会按列名结构化提取，列名不能改写）：\n"
        "- 一、事件现场基本情况：字段、内容（5 行：事件地点/天气情况/事件简述/周边环境/主要影响）\n"
        "- 二、预案匹配与组织预警和响应：字段、内容（5 行：匹配预案/事件等级/预警发布/启动响应/判断依据）\n"
        "- 三、应急组织机构：工作组、牵头单位、主要职责（至少 7 行）\n"
        "- 四、物资装备与调度：所需物资、推荐调度来源、距离、预计到达时间、地点、联系人信息、资源缺口（6 列）\n"
        "- 五、处置流程建议（包括后期处置、新闻发布）：序号、行动、责任单位、协同单位、引用依据（5 列，至少 10 行）\n"
        "- 六、次生风险：触发条件、风险描述、影响后果、应对措施、责任单位（5 列，至少 5 条）\n"
        "- 七、引用依据：依据类型、依据名称、引用章节/模块、引用内容摘要、支撑决策（5 列）；引用章节/模块列必须填 hit_path（如 '应急响应.处置措施.Ⅱ级应急响应处置措施'）\n\n"
        f"【审核发现的问题】\n{issue_block}\n\n"
        f"【审核建议】\n{advice_block}\n\n"
        f"【上一版候选最终方案】\n{candidate_text}"
    )
    ...
    await cl.Message(
        content="\n".join(
            [
                ...
                f"- 本地目录：`{run_dir}`",
                f"- 证据包：`{evidence_path}`",
                "- 生成方式：7 个章节逐章生成、逐章审核；每章完成后会立即展示。",
            ]
        ),
        author="章节生成流水线",
    ).send()
...
async def review_final_response_before_display(
    ...
    真正展示给用户的最终方案由 FinalPlanPipeline 按 7 个章节分别生成、
    ...
    # 如果 Pipeline 两次都失败了，用主模型生成一个符合 7 章节结构的详细方案作为兜底
    if pipeline_result is None:
...
async def _fallback_full_rewrite(agent: Agent, candidate_text: str) -> str:
    """Pipeline 完全失败时，用主模型兜底生成一个完整的 7 章节方案。"""
    section_text = "\n".join(f"- {heading}" for heading in STANDARD_PLAN_SECTIONS)
    fallback_prompt = (
        ...
        "硬性要求：\n"
        "1. 必须严格按 【应急处置总览】+【应急处置详情】结构 + 以下 7 个固定章节输出，不能增删、不能换序：\n"
        f"{section_text}\n\n"
        "2. 除第三节固定输出两张独立 Markdown 表格外，每个章节只输出规范要求的那张固定字段表（API 会按列名结构化提取，列名不能改写）：\n"
        "   - 一、事件现场基本情况：字段、内容（5 行：事件地点/天气情况/事件简述/周边环境/主要影响）\n"
        "   - 二、预案匹配与组织预警和响应：字段、内容（5 行：匹配预案/事件等级/预警发布/启动响应/判断依据）\n"
        "   - 三、应急组织机构：固定两张独立 Markdown 表格。第一张为工作组、牵头单位、主要职责（至少 7 个工作组：现场指挥组/综合协调组/抢险处置组/医疗救护组/后勤保障组/信息发布组/专家组）；第二张在'专家库支持'标题后输出专家姓名、所在单位、专业方向、联系电话、建议支持方式\n"
        "   - 四、物资装备与调度：所需物资、推荐调度来源、距离、预计到达时间、地点、联系人信息、资源缺口（6 列；基于实际工具数据）\n"
        "   - 五、处置流程建议（包括后期处置、新闻发布）：序号、行动、责任单位、协同单位、引用依据（5 列，至少 10 行）\n"
        "   - 六、次生风险：触发条件、风险描述、影响后果、应对措施、责任单位（5 列，至少 5 条）\n"
        "   - 七、引用依据：依据类型、依据名称、引用章节/模块、引用内容摘要、支撑决策（5 列；引用章节填 hit_path）\n\n"
        "3. 所有表格必须有表头行和分隔行（|---|---|），列数一致\n"
        "4. 只能用建议性表述，不能写已通知/已派遣/已下达指令\n"
        "5. 资源类别只能用中文名称\n"
        "6. 不要输出过程说明，直接输出标准化 7 章节方案\n"
                        ...
                        if set(recent_3) == set(prev_3):
                            ...
                            reminder = Message(
                                ...
                                content=(
                                    ...
                                    "请基于已有的全部工具结果，直接输出完整的标准化 7 章节应急指挥方案，"
                    ...
                    if (
                        ...
                        reminder = Message(
                            ...
                            content=(
                                ...
                                "请停止调用工具，直接基于已有的全部工具结果输出完整的标准化 7 章节应急指挥方案，"
                                "并在末尾附上 agent_control，设置 final_output=true。"
                            ),
                        )
                        agent.state.add_message(reminder)
