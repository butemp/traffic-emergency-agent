...
def build_output_format_retry_prompt() -> str:
    ...
    return (
        ...
        f"{section_text}\n"
        "2. 每个章节只输出规范要求的那张固定字段表，不要添加额外子表（'事件基础信息'/'补充说明'/'第一梯队/第二梯队'/'关键物资用途说明'/'资源覆盖与缺口分析'/'三阶段子表'/'安全/处置/衍生风险三类'等都已经被砍掉）；\n"
        "3. 表头列名严格不能改写，API 会按列名结构化提取（错一个字段就解析不出来）：\n"
        "   - 一：字段、内容（5 行：事件地点/天气情况/事件简述/周边环境/主要影响）\n"
        "   - 二：字段、内容（5 行：匹配预案/事件等级/预警发布/启动响应/判断依据）\n"
        "   - 三：工作组、牵头单位、主要职责（至少 7 个工作组：现场指挥组/综合协调组/抢险处置组/医疗救护组/后勤保障组/信息发布组/专家组）\n"
        ...
        "9. 如已有专家检索结果，必须在'三、应急组织机构'表格后追加'专家库支持'段，写出专家姓名、单位、专业方向和建议支持方式；\n"
...
def build_final_review_retry_prompt(
    ...
    return (
        ...
        "9. 如已有专家检索结果，必须在三、应急组织机构 表格后追加'专家库支持'段，写出专家姓名、单位、专业方向和建议支持方式；\n"
...
async def _fallback_full_rewrite(agent: Agent, candidate_text: str) -> str:
    ...
    fallback_prompt = (
        ...
        f"{section_text}\n\n"
        "2. 每个章节只输出规范要求的那张固定字段表（API 会按列名结构化提取，列名不能改写）：\n"
        "   - 一、事件现场基本情况：字段、内容（5 行：事件地点/天气情况/事件简述/周边环境/主要影响）\n"
        "   - 二、预案匹配与组织预警和响应：字段、内容（5 行：匹配预案/事件等级/预警发布/启动响应/判断依据）\n"
        "   - 三、应急组织机构：工作组、牵头单位、主要职责（至少 7 个工作组：现场指挥组/综合协调组/抢险处置组/医疗救护组/后勤保障组/信息发布组/专家组）\n"
